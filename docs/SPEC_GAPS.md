...
| [`PROPOSAL_LAYER_BLEND.md`](./PROPOSAL_LAYER_BLEND.md) | **Proposed** 2026-08-18 | Per-paint `blend:` on `Color()` / `Ramp()` / `MediaLayer()`; isolated bands (not pass-through); portable `BlendMode` eight. **L0–L1** language + HTML; **L2** fg isolation lift; **L3** frame `blendMode`. Not locked — do not treat syntax as normative. |

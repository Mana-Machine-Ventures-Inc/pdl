...
| [`PROPOSAL_REPEAT_NUMBER_BOUNDS.md`](./PROPOSAL_REPEAT_NUMBER_BOUNDS.md) | **Locked** 2026-08-18 | `Repeat(count:, begin: = 1)` bake mount (nested OK); `Number(min:, max:)` + `type Name = Number(…)`; page control = count + currentPage. Ceilings 32 / 64. Errors PDL-E057…E060. |
| [`PROPOSAL_LAYOUT_TWEEN.md`](./PROPOSAL_LAYOUT_TWEEN.md) | **Proposed** 2026-08-19 | Opt-in FLIP (`layout: .match`) for in-place rebake (toggle) and Presenter crossings (photo → detail); optional frame `match:` identity. Not default Smart Animate; not a new motion type. Not locked. |

...
| [`PROPOSAL_REPEAT_NUMBER_BOUNDS.md`](./PROPOSAL_REPEAT_NUMBER_BOUNDS.md) | **Proposed** 2026-08-18 | `Repeat(count:, begin: = 1)` bake mount (nested OK); `Number(min:, max:)` on params/types; page control = count + currentPage. No `Range` arg. Not locked. |

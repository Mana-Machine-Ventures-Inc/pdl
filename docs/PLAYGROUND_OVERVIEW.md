...
| **P6** | Incremental preview apply — param/interaction updates **bake IR → DOM reconcile** (primary); HTML morph fallback; cold path still `srcdoc`. Status shows `· live apply`. See [`PROPOSAL_INCREMENTAL_PREVIEW_APPLY.md`](./PROPOSAL_INCREMENTAL_PREVIEW_APPLY.md). |

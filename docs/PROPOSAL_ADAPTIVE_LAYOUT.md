...
**Status:** superseded (2026-08-16) by [`PROPOSAL_HOST_ENVIRONMENT.md`](./PROPOSAL_HOST_ENVIRONMENT.md) — **H0–H5 shipped**. Prefer pack `host Name(params) [mount]` + `<Host>` over a language-fixed `SizeClass` prelude. This doc remains useful for product rules (opt-in structure flips, no CSS in bake, measure the mount).  

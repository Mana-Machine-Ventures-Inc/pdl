...
(* Declared emit capture — handler assignment; not keyword `on`.
   ...
   Bare `animate =` here is land choreography for the rebake this capture causes; the scope is
   the capturing component's own tree. `emit` re-fire and targeted `IDENT.animate =` are not
   capture statements. *)
emit-capture-handler
...
animate-statement
  ::= 'animate' '=' animate-rhs
    | IDENT '.' 'animate' '=' animate-rhs
    | 'self' '.' 'animate' '=' animate-rhs
    ;

(* Handler land / choreography RHS — Pose-list sugar desugars to Animation(keys:). *)
animate-rhs
  ::= '[' pose-or-rest { ',' pose-or-rest } ']'
    | value-expr
    ;
...
(* Segment: clock + destination pose. `pose:` optional → `.rest` (handler land sugar:
   `animate = Motion(duration:, ease:)` desugars to Animation(keys: [that Motion])).
   Motion with a non-rest pose is not a valid bare `animate =` value. *)
motion-literal
  ::= 'Motion' '(' motion-args ')' ;
